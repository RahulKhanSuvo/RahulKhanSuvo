...
function ScrollVelocityRowImpl({
  ...
  const hoverProgressRef = useRef(1)
  ...
  useEffect(() => {
    ...
    if (container && block) {
      const updateSizes = () => {
        ...
        unitWidth.set(bw + gap)
        const nextCopies =
          bw > 0 ? Math.max(3, Math.ceil(cw / (bw + gap)) + 2) : 1
  ...
  useAnimationFrame((_, delta) => {
    ...
    const dt = delta / 1000
    if (pauseOnHover) {
      const target = isHoverRef.current ? 0 : 1
      const ease = 1 - Math.exp(-dt * 3)
      hoverProgressRef.current += (target - hoverProgressRef.current) * ease
      if (hoverProgressRef.current < 0.001) {
  ...
  return (
    <div
      ...
      onMouseEnter={pauseOnHover ? () => (isHoverRef.current = true) : undefined}
      onMouseLeave={pauseOnHover ? () => (isHoverRef.current = false) : undefined}
      {...props}
    >
      <motion.div
        className="inline-flex transform-gpu items-center will-change-transform select-none"
        style={{ x, gap: gap > 0 ? gap : undefined }}
      >
